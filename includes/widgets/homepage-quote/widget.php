...
if ( !class_exists('homepage_quote') ):

class homepage_quote extends WP_Widget {
  public $template = 'includes/widgets/homepage-quote/template.php';

  function homepage_quote() {
    $widget_ops = array(
      'classname' => 'widget_id_homepage_quote',
      'description' => __( "Title and text for Homepage with Quote", 'idiit-theme'),
      'panels_groups' => array('id-theme')
    );
    parent::__construct( false, __('ID Homepage Quote', 'idiit-theme'), $widget_ops );
  }
...
<section id="widget_id_homepage_quote_adm<?php echo $this->number ?>">
    <p>
	...
	<div class="siteorigin-widget-field siteorigin-widget-field-type-tinymce siteorigin-widget-field-text">				
	  ...
	  <div class="siteorigin-widget-tinymce-container"
		...
		$settings = array( 'media_buttons' => false, 'textarea_name' => $this->get_field_name( 'custom_summary' ));
   ...
   <script>
      jQuery(document).ready(function($){
        $('#widget_id_homepage_quote_adm<?php echo $this->number ?> .checkbox-controller').each(function( index, element ){
          if ($(this).is(":checked")) {
		...
		/*tinyMCE.init({
            selector: '#widget_id_homepage_quote_adm<?php echo $this->number ?> textarea',
            height : "100",
            dialog_type:"modal",
        ...
        $('#widget_id_homepage_quote_adm<?php echo $this->number ?> .checkbox-controller').on('change', function(){
          if ($(this).is(":checked")) {
            $(this).parent().parent().find('.tohide').show();
          }else{
            $(this).parent().parent().find('.tohide').hide();
...
add_action( 'widgets_init', function() {
  register_widget( 'homepage_quote' );
});
