...
if ( !class_exists('Editor_Hero') ):

  class Editor_Hero extends WP_Widget {
    public $template = 'includes/widgets/editor-herounit/template.php';

    function Editor_Hero() {
      $widget_ops = array(
        'classname' => 'widget_Editor_Hero_ctm',
        'description' => __( "Display the post featured image or slider", 'idiit-theme'),
        'panels_groups' => array('id-theme')
      );
      parent::__construct( false, __('Post Featured Image', 'idiit-theme'), $widget_ops );
    }
...
<!--       <section id="Editor_Hero_ctm_adm">
        <div>
          <p>
      ...
      <script>
        jQuery(document).ready(function($){
          $('#Editor_Hero_ctm_adm .checkbox-controller').each(function( index, element ){
            if ($(this).is(":checked")) {
          ...
          $('#Editor_Hero_ctm_adm .checkbox-controller').on('change', function(){
            console.log('post');
            if ($(this).is(":checked")) {
              $(this).parent().parent().find('.tohide').show();
            }else{
