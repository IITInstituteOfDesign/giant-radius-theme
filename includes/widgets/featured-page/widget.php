...
if ( !class_exists('Featured_Page') ):

class Featured_Page extends WP_Widget {
  public $template = 'includes/widgets/featured-page/template.php';

  function Featured_Page() {
    $widget_ops = array(
      'classname' => 'widget_featured_page_ctm',
      'description' => __( "Highlighted Page", 'idiit-theme'),
      'panels_groups' => array('id-theme'),
    );
    parent::__construct( false, __('ID Highlighted Page', 'idiit-theme'), $widget_ops );
...
<section id="widget_featured_page_ctm_adm">
    <p>
      ...
      <label for="<?php echo $this->get_field_id('show_img'); ?>">Show Page Featured Image</label>
    </p>
    <p>
   ...
   <script>
      jQuery(document).ready(function($){
        $('#widget_featured_page_ctm_adm .checkbox-controller').each(function( index, element ){
          if ($(this).is(":checked")) {
        ...
        $('#widget_featured_page_ctm_adm .checkbox-controller').on('change', function(){
          console.log('page');
          if ($(this).is(":checked")) {
            $(this).parent().parent().find('.tohide').show();
          }else{
